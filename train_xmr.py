...
def train_model(root, term_size_map, term_direct_gene_map, dG, train_data, gene_dim, model_save_folder, train_epochs, batch_size, learning_rate, num_hiddens_genotype, num_hiddens_drug, num_final_drug, num_hiddens_final, cell_features, drug_train, drug_test, N_fingerprints, pathway):
    ...
    for epoch in range(train_epochs):
        ...
        train_corr = spearman_corr(train_predict, train_label_gpu)
        test_corr = test_acc(model, test_loader, batch_size, drug_test, test_label_gpu, gene_dim, cuda_cells, CUDA_ID)
        print(">>>>>Training step %d: model training acc %f test acc %f" % (epoch, train_corr, test_corr))
        if test_corr >= max_corr:
            max_corr = test_corr
            best_model = epoch
            torch.save(model.state_dict(), f"./{model_save_folder}/{pathway}_best.pkl")
        
        print("Best performed model (epoch)\t%d" % best_model)

    torch.save(model.state_dict(), f"./{model_save_folder}/{pathway}_final.pkl")
